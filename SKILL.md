...
name: iflow-to-im
description: |
  Bridge THIS iFlow CLI session to Telegram, Discord, Feishu/Lark, QQ, or WeCom so the
  user can chat with iFlow from their phone. Use for: setting up, starting, stopping,
  or diagnosing the iflow-to-im bridge daemon; forwarding iFlow replies to a messaging
  app; any phrase like "iflow-to-im", "bridge", "消息推送", "消息转发", "桥接",
  "连上飞书", "手机上用iflow", "启动后台服务", "诊断", "查看日志", "配置".
...
# iFlow-to-IM Bridge Skill

You are managing the iFlow-to-IM bridge.
User data is stored at `~/.iflow-to-im/`.

The skill directory (SKILL_DIR) is at `~/.iflow/skills/iflow-to-im`.
If that path doesn't exist, fall back to Glob with pattern `**/skills/**/iflow-to-im/SKILL.md` and derive the root from the result.
...
|---|---|
| `setup`, `configure`, `配置`, `我想在飞书上用 iFlow`, `帮我连接 Telegram` | setup |
...
1. **iFlow CLI** — `AskUserQuestion` tool is available. Use it for interactive setup wizards.
2. **Codex / other** — `AskUserQuestion` is NOT available. Fall back to non-interactive guidance: explain the steps, show `SKILL_DIR/config.env.example`, and ask the user to create `~/.iflow-to-im/config.env` manually.
...
Before running any subcommand other than `setup`, check if `~/.iflow-to-im/config.env` exists:

- **If it does NOT exist:**
  - In iFlow CLI: tell the user "No configuration found" and automatically start the `setup` wizard using AskUserQuestion.
  - In Codex: tell the user "No configuration found. Please create `~/.iflow-to-im/config.env` based on the example:" then show the contents of `SKILL_DIR/config.env.example` and stop. Don't attempt to start the daemon — without config.env the process will crash on startup and leave behind a stale PID file that blocks future starts.
...
Ask which channels to enable (telegram, discord, feishu, qq, wecom). Accept comma-separated input. Briefly describe each:
...
- **wecom** (企业微信) — WeCom enterprise chat. Supports application messaging with interactive buttons.
...
- **WeCom (企业微信)**: Only two fields needed:
  1. Bot ID (required) → confirm
  2. Bot Secret (required) → confirm (masked)
  - Tell the user: These values can be found at 企业微信管理后台
  3. Allowed User IDs (optional) — can leave empty to allow all users
  - **Keep it simple** — don't ask for Corp ID, Agent ID, Token, or EncodingAESKey unless the user specifically mentions them

**Step 3 — General settings**

Ask for runtime, default working directory, model, and mode:
- **Runtime**: `iflow` (default), `codex`, `auto`
  - `iflow` — uses iFlow CLI (requires `iflow` CLI installed)
  - `codex` — uses OpenAI Codex SDK (requires `codex` CLI; auth via `codex auth login` or `OPENAI_API_KEY`)
  - `auto` — tries iFlow first, falls back to Codex if not found
...
3. Use Bash to create directory structure: `mkdir -p ~/.iflow-to-im/{data,logs,runtime,data/messages}`
4. Use Write to create `~/.iflow-to-im/config.env` with all settings in KEY=VALUE format
5. Use Bash to set permissions: `chmod 600 ~/.iflow-to-im/config.env`
...
8. On success, tell the user: "Setup complete! Run `/iflow-to-im start` to start the bridge."

### `start`

**Pre-check:** Verify `~/.iflow-to-im/config.env` exists (see "Config check" above). Without it, the daemon will crash immediately and leave a stale PID file.

Run: `bash "SKILL_DIR/scripts/daemon.sh" start`

Show the output to the user. If it fails, tell the user:
- Run `doctor` to diagnose: `/iflow-to-im doctor`
- Check recent logs: `/iflow-to-im logs`
...
1. Read current config from `~/.iflow-to-im/config.env`
...
7. Remind user: "Run `/iflow-to-im stop` then `/iflow-to-im start` to apply the changes."
...
- Config persists at `~/.iflow-to-im/config.env` — survives across sessions.